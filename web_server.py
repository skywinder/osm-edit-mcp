...
This legacy wrapper exposes read-only helpers on loopback. OSM writes use MCP.
"""
...
# The legacy HTTP wrapper is deliberately local/read-only. It refuses all
# authenticated routes unless an operator supplies a non-default secret.
API_KEY = os.getenv("API_KEY")


def _tool_result(result: object) -> object:
    """Preserve an MCP tool's real success/failure result."""
    return result
...
def verify_api_key(credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    if not API_KEY:
        raise HTTPException(
            status_code=503, detail="HTTP API authentication is not configured"
        )
...
# Configure CORS. An empty origin list means browser access is disabled.
app.add_middleware(
    CORSMiddleware,
...
async def api_find_nearby_amenities(
    request: NearbyAmenitiesRequest, api_key: str = Depends(verify_api_key)
):
...
async def api_get_place_info(
    request: PlaceInfoRequest, api_key: str = Depends(verify_api_key)
):
    """Get information about a place"""
    try:
        result = await get_place_info(place_name=request.query)
        return _tool_result(result)
    except Exception as e:
...
async def api_smart_geocode(
    request: GeocodeRequest, api_key: str = Depends(verify_api_key)
):
    """Convert address to coordinates"""
    try:
        result = await smart_geocode(address_or_description=request.query)
        return _tool_result(result)
    except Exception as e:
...
async def api_validate_coordinates(
    request: CoordinatesRequest, api_key: str = Depends(verify_api_key)
):
    """Validate coordinates"""
    try:
        result = await validate_coordinates(lat=request.lat, lon=request.lon)
        return _tool_result(result)
    except Exception as e:
...
async def api_get_osm_elements_in_area(
    request: AreaRequest, api_key: str = Depends(verify_api_key)
):
    """Get all OSM elements in an area"""
    try:
        result = await get_osm_elements_in_area(
            bbox=(
                f"{request.min_lon},{request.min_lat},"
                f"{request.max_lon},{request.max_lat}"
            )
        )
        return _tool_result(result)
    except Exception as e:
...
async def api_parse_natural_language(
    request: NaturalLanguageRequest, api_key: str = Depends(verify_api_key)
):
...
async def api_get_osm_node(node_id: int, api_key: str = Depends(verify_api_key)):
    """Get OSM node by ID"""
    try:
...
async def api_get_osm_way(way_id: int, api_key: str = Depends(verify_api_key)):
    """Get OSM way by ID"""
    try:
...
async def api_get_osm_relation(
    relation_id: int, api_key: str = Depends(verify_api_key)
):
...
async def api_create_changeset(comment: str, api_key: str = Depends(verify_api_key)):
    """Direct HTTP writes are retired in favor of MCP proposals."""
    raise HTTPException(
        status_code=410,
        detail="Direct HTTP writes are disabled; use the MCP preview/apply workflow",
    )


@app.post("/api/node")
async def api_create_osm_node(
    request: CreateNodeRequest, api_key: str = Depends(verify_api_key)
):
    """Direct HTTP writes are retired in favor of MCP proposals."""
    raise HTTPException(
        status_code=410,
...
async def api_create_place_from_description(
    request: CreatePlaceRequest, api_key: str = Depends(verify_api_key)
):
    """Direct HTTP writes are retired in favor of MCP proposals."""
    raise HTTPException(
        status_code=410,
